...
(describe "A TicTacToe Game-mode"
  ...
  (context "PvPGame"
    ...
    (it "gets the initial board"
      (should= (utils/empty-board 3) (initial-board (->PvPGame 3))))

    (it "gets the next board"
      (with-redefs [read-line (stub :read-line {:return "1"})]
        (should= [\x \_ \_ \_ \_ \_ \_ \_ \_]
                 (next-board (->PvPGame 3) (utils/empty-board 3)))
        (should-have-invoked :read-line))

      (with-redefs [read-line (stub :read-line {:return "2"})]
        (should= [\x \o \_ \_ \_ \_ \_ \_ \_]
                 (next-board (->PvPGame 3) [\x \_ \_ \_ \_ \_ \_ \_ \_]))
        (should-have-invoked :read-line))))

  (context "PvCGame"
    (it "gets the initial board"
      (should= (utils/empty-board 3) (initial-board (->PvCGame 3 \x :hard)))
      (should= [\_ \_ \_ \_ \_ \_ \_ \_ \x] (initial-board (->PvCGame 3 \o :hard))))

    (it "gets the next board"
      (with-redefs [read-line (stub :read-line {:return "1"})]
        (should= [\x \_ \_ \_ \_ \_ \_ \_ \_]
                 (next-board (->PvCGame 3 \x :hard) (utils/empty-board 3)))
        (should-have-invoked :read-line))
      ...
      (should= (move/play-move (utils/empty-board 3) (move/get-computer-move :hard (utils/empty-board 3)))
               (next-board (->PvCGame 3 \o :hard) (utils/empty-board 3))))))